...
var POST_SUBMIT_REQUEST = exports.POST_SUBMIT_REQUEST = 'wordpress-redux/post-submit/REQUEST';
var POST_SUBMIT_REQUEST_SUCCESS = exports.POST_SUBMIT_REQUEST_SUCCESS = 'wordpress-redux/post-submit/REQUEST_SUCCESS';
var POST_SUBMIT_REQUEST_FAILURE = exports.POST_SUBMIT_REQUEST_FAILURE = 'wordpress-redux/post-submit/REQUEST_FAILURE';
...
function submitPost(post, type) {
  return function (dispatch) {
    ...
    return submitPostRequest(post, type).then(function (data) {
      dispatch({
        type: POST_SUBMIT_REQUEST_SUCCESS,
        post: data,
        postId: post.id
      });
      return data;
      ...
      dispatch({
        type: POST_SUBMIT_REQUEST_FAILURE,
        postId: post.id,
        error: error
      });
      return error;
...
function submitPostRequest(data, type) {
  var url = api.config.url + 'wp-json/wp/v2/' + type;

  var headers = {
    'Accept': 'application/json',
    'X-WP-Nonce': SiteSettings.nonce,
    'Authorization': 'Basic Y29udHJpYnV0ZXI6cnhDQjJFSHdqT294SXVhT1lodThvcDdq',
		'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
  };
 ...
 * @return {Function}           Action thunk
 */
function submitImage(filename, file) {
  return function (dispatch) {
    dispatch({
      type: POST_SUBMIT_REQUEST,
    });

    return submitImageRequest(filename, file).then(function (data) {
      dispatch({
        type: POST_SUBMIT_REQUEST_SUCCESS,
        post: data,
      });
...
// Helper to submit the comment with nonce header
function submitImageRequest(filename, file) {

  var url = api.config.url + 'wp-json/wp/v2/media';
  ...
  return fetch(url, {
    method: 'POST',
    headers: headers,
    body: _qs2.default.stringify(file),
    mode: 'same-origin',
    credentials: 'include'
  }).then(function (response) {
    return response.text().then(function (text) {
